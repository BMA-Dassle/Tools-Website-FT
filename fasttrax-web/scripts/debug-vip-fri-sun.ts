...
async function checkAvailability(centerId: number, offerId: number, date: string, token: string) {
  ...
  for (const t of probes) {
    ...
    const data = JSON.parse(text) as { Availabilities?: Array<{ BookedAt: string; WebOffer: { Id: unknown; Options?: Record<string, unknown[]> } }> };
    ...
    for (const a of avails) {
      ...
      console.log(`  ${t}  BookedAt=${a.BookedAt}  WebOffer.Id=${JSON.stringify(wid)}(${widType})  ==offerId:${matchNum}  matchAsStr:${matchStr}  opts=${opts}`);
    }
...
async function main() {
  ...
  for (const r of expRows) {
    ...
    console.log(`  slug=${row.slug}  id=${row.id}  kind=${row.kind}  vip=${row.is_vip}  active=${row.is_active}  days=${JSON.stringify(row.days_of_week)}`);
  }
  ...
  for (const r of offerRows) {
    const row = r as Record<string, unknown>;
    console.log(`  slug=${row.slug}  center=${row.center_code}  qamfOfferId=${row.qamf_web_offer_id}  optType=${row.qamf_option_type}  active=${row.is_active}`);
  }
  ...
  for (const r of durRows) {
    const row = r as Record<string, unknown>;
    console.log(`  slug=${row.slug}  center=${row.center_code}  optId=${row.qamf_option_id}  mins=${row.duration_minutes}  label=${row.label}  mult=${row.square_multiplier}`);
  }
  ...
  for (const r of itemRows) {
    const row = r as Record<string, unknown>;
    console.log(`  slug=${row.slug}  catalogId=${row.square_catalog_object_id}  qty=${row.quantity}  product="${row.product_label}"  price=${row.price_cents}  productActive=${row.product_active}`);
  }
