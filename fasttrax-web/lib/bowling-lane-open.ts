...
export async function processLaneOpen(opts: {
  ...
  if (reservation.squareDayofOrderId) {
    ...
    if (!orderRes.ok) {
      ...
      console.error(`[lane-open] neonId=${neonId} order fetch failed: ${msg} (retryable=${transient})`);
      ...
      if (reservation.squareGiftCardId) {
        try {
          ...
          if (gcBalance > 0) {
            ...
            const tPay = Date.now();
            const payRes = await fetchWithRetry(`${SQUARE_BASE}/payments`, {
              method:  "POST",
              headers: sqHeaders(),
              body:    JSON.stringify({
            ...
            if (payRes.ok) {
              ...
              console.error(`[lane-open] neonId=${neonId} gift card payment failed: ${errMsg} (retryable=${retryable})`);
          ...
          console.error(`[lane-open] neonId=${neonId} payment threw: ${errMsg} (retryable=${retryable})`);
          processingError = errMsg;
        }
      }
  ...
  console.log(`[lane-open] neonId=${neonId} src=${srcTag} Neon write ${Date.now() - tNeon}ms retryable=${retryable && !!processingError}`);
