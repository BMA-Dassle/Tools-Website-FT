 ...
 *   centerId    — "9172" | "3148"
 *   depositPaid — cents charged at booking (integer string, may be absent)
 *   remaining   — cents due at center      (integer string, may be absent)
 *
 * Data source: GET /api/bowling/v2/reservations/[neonId]
 * Falls back gracefully when neonId=0 or the fetch fails.
 *
 * Parallel deployment — sits at /hp/book/kids-bowl-free-v2/confirmation/.
 * Moves to /hp/book/kids-bowl-free/confirmation/ at PR 2 cutover.
 */
...
function ConfirmationContent() {
  ...
  // Prefer Neon data; fall back to URL params
  ...
  // Lines come from Neon; if unavailable fall back to empty
      ...
      <main className="pt-28 sm:pt-36 pb-24 px-4">
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-7 space-y-3">
            {qamfId && (
              <Row label="Booking ref" value={qamfId} mono />
            )}
            <Row label="Center" value={centerName} />
            {centerAddress && (
              <Row label="Address" value={centerAddress} />
            )}
            {dateLabel && <Row label="When" value={dateLabel} />}
            {playerCount != null && (
              <Row label="Bowlers" value={String(playerCount)} />
            )}
            {guestName && <Row label="Guest" value={guestName} />}

            {/* Line items (shoes, add-ons) */}
            {lines.length > 0 && (
              <>
                <DividerLine />
                <div>
            ...
            {/* Payment summary — only shown when there's a charge */}
            {hasPaidDeposit && (
              <>
                <DividerLine />
                <div className="space-y-1.5">
                  {displayTotal > 0 && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white">{centsToDollars(displayTotal)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm">
                    <span className="text-white/50">Paid at booking</span>
                    <span style={{ color: "#4ade80" }}>{centsToDollars(displayDepositPaid)}</span>
                  </div>
                  {displayRemaining > 0 && (
                    <div className="flex justify-between text-sm font-semibold">
                      <span className="text-white/80">Balance due at center</span>
                      <span className="text-white">{centsToDollars(displayRemaining)}</span>
                    </div>
                  )}
                  {displayRemaining === 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-white/50">Balance due at center</span>
                      <span className="text-white/50">Paid in full</span>
                    </div>
                  )}
                </div>
              </>
          ...
          >
            ...
            <ul className="text-white/75 text-sm space-y-1 list-disc list-inside">
              ...
              {hasPaidDeposit && displayRemaining > 0 && (
                <li>
...
export default function KbfV2ConfirmationPage() {
  return (
    <Suspense
      fallback={
        <div style={{ backgroundColor: BG }} className="min-h-screen">
