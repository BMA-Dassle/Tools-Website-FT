...
- **Staff admin proxy** at `apps/admin/` — a tiny Next app (its own Vercel project, Root Directory `apps/admin`, behind Vercel Authentication) that serves the admin tools at clean URLs (`/pit`, `/videos`, …) by PROXYING every request to the main deployment with `ADMIN_CAMERA_TOKEN` injected into the path. It owns no pages, no API routes, no vercel.json (so no crons) and no secrets beyond the token + `ADMIN_PROXY_KEY`; routing table in `apps/admin/src/routes.ts` (drift-pinned to `apps/web/app/admin/[token]/*` by its test). The main middleware accepts `x-admin-proxy-key` as an admin credential (inert until `ADMIN_PROXY_KEY` is set). Client-side URLs that guests/devices must reach (VIP QR, TV URLs) go through `publicOrigin()` (`~/lib/helpers/public-origin`, host-based) — see tasks/lessons.md § Serving origin ≠ public origin.
