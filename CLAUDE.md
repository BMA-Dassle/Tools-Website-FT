...
- **Staff admin proxy** at `apps/admin/` — a tiny Next app (its own Vercel project, Root Directory `apps/admin`, domain `admin.fasttraxent.com`) that serves the admin tools at clean URLs (`/pit`, `/videos`, …) by PROXYING every request to the main deployment with `ADMIN_CAMERA_TOKEN` injected into the path. **Auth is Microsoft SSO via the HeadPinz gateway** (Auth.js v5 in `apps/admin/auth.ts`, wired into `proxy.ts`); it replaced Vercel Authentication 2026-08-28. The only routes it serves itself are `/api/auth/*` and `/sso/*` (the `self` decision in `src/routes.ts`); everything else is proxied. Routing table in `apps/admin/src/routes.ts`, drift-pinned to `apps/web/app/admin/[token]/*` by its test. The main middleware accepts `x-admin-proxy-key` as an admin credential (inert until `ADMIN_PROXY_KEY` is set), and the shell forwards `x-sso-email` / `x-sso-name` upstream — **stripping any inbound copies first** (`IDENTITY_HEADERS` in `proxy.ts`), so those headers are the shell's word or absent, never the visitor's. Env: `SSO_ISSUER`, `SSO_CLIENT_ID=fasttrax-admin`, `SSO_CLIENT_SECRET`, `AUTH_SECRET`, `DIAG_SECRET`, `ADMIN_PROXY_KEY`, `ADMIN_CAMERA_TOKEN`, `ADMIN_UPSTREAM_ORIGIN` — **the four SSO vars must be set on the Vercel project BEFORE this branch deploys**, or Auth.js throws on every request and the shell 500s everything (tasks/todo.md § Runbook). Troubleshoot with `GET /sso/diag` (Bearer `DIAG_SECRET`). Client-side URLs that guests/devices must reach (VIP QR, TV URLs) go through `publicOrigin()` (`~/lib/helpers/public-origin`, host-based, and it drops any `admin.*` host) — see tasks/lessons.md § Serving origin ≠ public origin.
- **Admin credentials in `apps/web`** — the static `ADMIN_CAMERA_TOKEN` must NEVER reach a browser. Pages mint a short-lived signed token (`lib/admin-api-token.ts`) and pass THAT to their client components — it authenticates **`/api/admin/*` only**; it is not a page credential, and the middleware branch that accepts it is scoped to that prefix (a browser-held credential that also rendered pages would defeat the PR 2 lockdown); route handlers check credentials through `lib/admin-request-auth.ts`, never with their own `token === process.env.…` comparison; staff-facing links are built with `adminToolUrl()` (`~/lib/helpers/admin-url`), never with `/admin/${token}/…`. `scripts/check-admin-token-leak.mjs` (runs in `npm run test -w fasttrax-web`) fails the build if a client-reachable module reads the token env. Full classification of every token usage: [tasks/admin-sso-lockdown.md](tasks/admin-sso-lockdown.md).
...
- **ALWAYS persist guest-provided data to our own DB (Neon) at capture — independent of, and not gated on, the external API call.** Anything a guest gives us that we send to Square / QAMF (Conqueror) / BMI (Pandora) MUST also be saved to Neon at the moment of capture, so an API failure never loses recoverable data. Our DB is the source of truth; external APIs are downstream syncs. Persistence is the first guaranteed step, never "best-effort after the API call." _(Lesson 2026-06-21 — Pizza Bowl toppings/soda were sent only to the Square order, never stored in Neon; when they failed to attach, the data was unrecoverable. See [tasks/lessons.md § Persist guest input](tasks/lessons.md).)_
...
- **FLAGS ARE KILL SWITCHES ONLY — never opt-in gates.** A merged feature is ON. If a flag exists at all it defaults ON (`!== "false"`) as an emergency off switch; never ship a feature dark behind `=== "true"` env vars. Not ready to be on = not ready to merge — keep it on a branch and test via its preview deployment. _(Owner rule 2026-07-31 — "if we do flags, it's only to turn features off; I don't want to fight it 24/7." Born from the split-tender rollout: two stacked opt-in flags + build-baked NEXT_PUBLIC vars cost three rounds of "did you redeploy?".)_
- **ALWAYS add BOTH English AND Spanish for any guest-facing kiosk copy — no hardcoded user-visible strings.** Every guest-facing string on the kiosk goes through the i18n catalog (`apps/web/src/features/kiosk/i18n/messages/`): add the key to the English source (`en.ts` or the matching `messages/parts/*.ts`) AND its Spanish counterpart in the same commit. `MessageKey` is `keyof` the English catalog and `es` is typed `Record<CoreKey, string>`, so a missing ES entry fails `tsc` — do not "fill it in later." This includes copy that lives in DATA, not just components (combo marketing copy, waiver body, tile labels): give the data an `es` field and have the component pick it when `locale === "es"`. Brand/product nouns stay English (FastTrax, HeadPinz, Game Zone, Podium, Duckpin). Admin/staff-only surfaces (`/kiosk/admin`, device/card-reader/QR) are exempt. _(Owner rule 2026-07-26 — the kiosk serves a ~20-27% Spanish-speaking SWFL audience; a half-translated screen is worse than none.)_
...
| [tasks/admin-sso-lockdown.md](tasks/admin-sso-lockdown.md) | Admin SSO audit — every admin-token usage classified, PR 2 checklist | Before touching ANY admin auth, token, or `/admin/…` link   |
