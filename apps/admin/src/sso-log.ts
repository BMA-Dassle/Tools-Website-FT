/**
 * Sign-in telemetry for the shell — an in-memory ring buffer plus one JSON
 * line per event.
 *
 * WHY NOT pino HERE. `auth.ts` is imported by `proxy.ts`, which runs in the
 * EDGE runtime, and pino is a Node library. So this module is a deliberately
 * tiny JSON-to-stdout shim — Vercel's runtime logs treat one JSON object per
 * line exactly the same whichever library wrote it. pino is used in
...
export function logSso(
  ...
  const line = JSON.stringify({ level, event, app: "ft-admin-proxy", ...detail });
