...
export default function BookRacePage() {
  ...
  const [licenseSold, setLicenseSold] = useState<{
    quantity: number;
  ...
  const [linkedSelected, setLinkedSelected] = useState<{
    id: string;
    name: string;
    age: number | null;
  } | null>(null);
  ...
  const [pendingHeat, setPendingHeat] = useState<{ proposal: BmiProposal; block: BmiBlock } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    for (const [packageId, pkgBookings] of packageBuckets) {
      ...
      const earliestStart = pkgBookings.map((b) => b.block.start).sort()[0] || baseDate;
      racingItems.push({
        attraction: "racing",
        attractionName: "Racing",
        product: {
    ...
    const isRookieBundle =
      !anyPackageActive && !!(selectedPov && selectedPov.quantity > 0 && selectedPov.rookiePack);
    ...
    console.log(
      "[cart sync] racerAssignments:",
      assignments.length,
      "bookings:",
      bookings.length,
      "verifiedRacers:",
      verifiedRacers.length,
    );
  ...
  async function fetchRacerCredits(person: PersonData): Promise<PersonData> {
    try {
      ...
      const racingCredits = deposits.filter(
        (d) =>
          d.balance > 0 &&
  ...
  function handlePersonVerified(person: PersonData) {
    ...
    fetchRacerCredits(person).then((updated) => {
      setVerifiedRacers((prev) =>
        prev.map((r) =>
          r.personId === updated.personId
            ? { ...r, hasCredits: updated.hasCredits, creditBalances: updated.creditBalances }
            : r,
        ),
      );
    });
    ...
    fetch(`/api/pandora?personId=${person.personId}&picture=false&allRelated=true`)
      .then(async (res) => {
        ...
        if (typeof data.valid === "boolean") {
          setVerifiedRacers((prev) =>
            prev.map((r) =>
              r.personId === person.personId ? { ...r, waiverValid: data.valid } : r,
            ),
          );
        }
        ...
        setLinkedPersons(
          related.filter(
            (
              p,
            ): p is { id: string; firstName: string; lastName: string; birthdate: string | null } =>
              p !== null,
          ),
        );
        setLinkedFetching(false);
  ...
  function handlePackageAfterWarning(pkg: PackageDefinition) {
    if (pkg.disclaimers && pkg.disclaimers.ackKeys.length > 0) {
    ...
    for (const b of targetBookings) {
      if (b.billLineIds && b.billLineIds.length > 0) {
        for (const ll of b.billLineIds) {
          try {
            await removeBookingLine(ll.billId, ll.lineId);
          } catch {
            /* non-fatal */
          }
        }
  ...
  async function handlePackageHeatsConfirm({ picks }: { picks: PackagePick[] }) {
    ...
    try {
      for (const pick of picks) {
        ...
        const race: ClassifiedProduct = {
          ...
          name:
            pick.component.tracks.length > 1
    ...
    if (selectedPackage.includesPov && billId) {
      try {
        ...
        const povLineId = povResult?.parentBillLineId
          ? String(povResult.parentBillLineId)
          : undefined;
        ...
        prev.some((b) => b.billId === result.billId)
          ...
          : [
              ...prev,
              {
                ...
                racerName: "Pack",
                category: selectedProduct?.category ?? "adult",
              },
            ],
      );
  ...
  async function handleConfirmHeat(proposal: BmiProposal, block: BmiBlock) {
    ...
    console.log("[handleConfirmHeat]", {
      racerType,
      verifiedRacersCount: verifiedRacers.length,
  ...
  async function bookHeatForRacers(
    proposal: BmiProposal,
    block: BmiBlock,
    selectedRacers: PersonData[] | null,
  ) {
    ...
    try {
      ...
      const existingBillId =
        activeBills.length > 0
          ? activeBills[0].billId
          : sessionStorage.getItem("attractionOrderId") || null;
      ...
      if (selectedRacers && selectedRacers.length > 0) {
        ...
        console.log(
          "[bookHeatForRacers] returning racers:",
          selectedRacers.map((r) => r.fullName).join(", "),
          "on bill:",
          orderId,
        );
      } else {
      ...
      console.log("[license check]", {
        ...
        selectedRacersLen: selectedRacers?.length,
        racerType,
        licenseSold: licenseSoldRef.current,
        primaryBillId,
      });
      ...
      const booking: Booking = {
        ...
        blockPrice,
        bills:
          createdBills.length > 0 ? createdBills : activeBills.filter((b) => b.category === cat),
  ...
  async function handleAddAnother(proposal: BmiProposal, block: BmiBlock) {
    ...
    try {
      ...
      const usedBills = existingBillId
        ? activeBills
        : [{ billId: rawOrderId, racerName: "Group", category: cat } as RacerBill];
  ...
  const filteredProducts = racerType
    ? filterProducts(
        ...
        adults,
        juniors,
        getCategoryMemberships(bookingCategory),
  ...
  return (
    <div className="min-h-screen bg-[#000418]">
      ...
      <div ref={contentRef} className="max-w-4xl mx-auto px-4 py-8 scroll-mt-[180px]">
            ...
            {verifiedRacers.length > 0 && !verifiedRacers[0]?.category && (
              <div className="max-w-md mx-auto rounded-xl border border-[#8652FF]/30 bg-[#8652FF]/5 p-4 space-y-3">
                <p className="text-white font-semibold text-sm text-center">
                  {verifiedRacers[0].fullName}, are you racing as...
                </p>
                  ...
                  <div className="space-y-2">
                    {linkedFetching && (
                      <div className="flex items-center justify-center gap-2 py-3 rounded-xl border border-dashed border-[#8652FF]/20">
                        ...
                        <span className="text-[#8652FF]/50 text-xs font-semibold">
                          ...
                          <div className="space-y-3">
                            <div className="rounded-lg border border-green-500/30 bg-green-500/5 p-3 text-center">
                              <p className="text-green-400 text-sm font-semibold">
                                {addingFoundPerson.fullName}
                              </p>
                              {addingAge !== null && (
                                <p className="text-white/40 text-xs">Age: {addingAge}</p>
                              )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <button
                                onClick={() => {
                                  handleAddRacer(addingFoundPerson, "adult");
                                  setAddingRacer(false);
                                  setAddingCategory(null);
                                  setAddingFoundPerson(null);
                                }}
                              ...
                              >
                                <span className="block">Adult</span>
                                <span
                                  className={`text-xs ${isUnder13 ? "text-white/10" : "text-white/30"}`}
                                >
                                  13+ · 59&quot;+ tall
                                </span>
                              </button>
                              <button
                                onClick={() => {
                                  handleAddRacer(addingFoundPerson, "junior");
                                  setAddingRacer(false);
                                  setAddingCategory(null);
                                  setAddingFoundPerson(null);
                                }}
                              ...
                              >
                                <span className="block">Junior</span>
                                <span
                                  className={`text-xs ${is13OrOver ? "text-white/10" : "text-white/30"}`}
                                >
                                  7-12 · 49&quot;+ tall
                                </span>
                              </button>
            ...
            {verifiedRacers[0]?.category && (
              <>
                <div className="max-w-md mx-auto rounded-xl border border-white/8 bg-white/3 p-3 text-xs text-white/40 text-center">
                  {verifiedRacers.length} racer{verifiedRacers.length !== 1 ? "s" : ""} in your
                  party
                </div>
                ...
                {bookings.map((b, i) => (
                  <div key={i} className="flex justify-between text-sm text-white/70">
                    ...
                    <span className="text-white/40">
                      {new Date(b.block.start).toLocaleTimeString("en-US", {
                        hour: "numeric",
                        minute: "2-digit",
                        hour12: true,
                      })}
                    ...
                    return (
                      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-center mb-4">
                        <p className="text-amber-400 text-xs">
                          Showing <strong>{tierLabel}</strong> and below for {bookingCategory}s —
                          not everyone has qualified for higher tiers.
                        </p>
                      </div>
                    );
                  })()}
        ...
        {disclaimerPending?.disclaimers && (
          <RaceWarningModal
            warning={disclaimerPending.disclaimers}
            onAcknowledge={() => {
                ...
                <div className="bg-[#0a1628] border border-white/15 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-5">
                  <div className="text-center">
                    ...
                    <p className="text-white/40 text-sm mt-2">
                      Just like roller coasters, height and age requirements are state mandated and
                      must be followed.
                    </p>
                  </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      const allChecked = disclaimers.every(
                        (_, i) =>
                          (document.getElementById(`height-ack-${i}`) as HTMLInputElement)?.checked,
                      );
                      if (allChecked) {
                        ...
                        disclaimers.forEach((_, i) => {
                          ...
                          if (cb && !cb.checked)
                            cb.parentElement?.classList.add("ring-2", "ring-red-500/50");
                        });
                      }
                    }}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
            <div className="bg-[#0a1628] border border-white/15 rounded-2xl p-6 max-w-md w-full shadow-2xl space-y-4">
              <div className="text-center">
                ...
                <p className="text-white/50 text-sm mt-1">
                  {bookings.length > 0 &&
                    `${bookings[bookings.length - 1]?.product.name} — ${bookings[bookings.length - 1]?.block.name}`}
                </p>
              </div>
              <div className="space-y-2">
                <button
            ...
            onContinue={async (pov) => {
              ...
              if (activeOrderId) {
                try {
                  const ovRes = await fetch(
                    `/api/sms?endpoint=bill%2Foverview&billId=${activeOrderId}`,
                  );
                  if (ovRes.ok) {
              ...
              if (pov && pov.quantity > 0 && activeOrderId) {
                try {
                  ...
                  const lineId = povResult.parentBillLineId
                    ? String(povResult.parentBillLineId)
                    : undefined;
                ...
                try {
                  const addonBody: Record<string, unknown> = {
                    ...
                    proposal: {
                      blocks: (
                        addon.proposal as {
                          blocks: { block: Record<string, unknown>; productLineIds?: string[] }[];
                        }
                      ).blocks.map((b) => ({
                      ...
                      productLineId:
                        (addon.proposal as { productLineId?: string }).productLineId ?? null,
                    },
                  };
                  const addonJson =
                    `{"orderId":${activeOrderId},` + JSON.stringify(addonBody).slice(1);
        ...
        {step === "summary" &&
          ...
          (packResult || bookings.length > 0) && (
            <OrderSummary
              ...
              verifiedRacers={verifiedRacers
                .filter((r) => r.personId)
                .map((r) => ({ personId: r.personId, fullName: r.fullName }))}
              ...
              onRemoveBooking={async (index) => {
                ...
                if (toRemove?.billLineIds && toRemove.billLineIds.length > 0) {
                  for (const { billId, lineId } of toRemove.billLineIds) {
                    ...
                    const otherBookingsOnBill = bookings.filter(
                      (b, i) => i !== index && b.bills?.some((bb) => bb.billId === billId),
                    );
                  ...
                  for (const bill of toRemove.bills) {
                    const otherBookingsOnBill = bookings.filter(
                      (b, i) => i !== index && b.bills?.some((bb) => bb.billId === bill.billId),
                    );
                    if (otherBookingsOnBill.length === 0) {
                ...
                try {
                  if (activeOrderId) {
                    ...
                    if (ovRes.ok) {
                      ...
                      console.log(
                        "[cancel rookie pack] POV scrub on bill",
                        activeOrderId,
                        "lines:",
                        povLines,
                      );
              ...
              <>
                ...
                {linkedPersons.map((lp) => {
                  ...
                  return (
                    <div
                      key={lp.id}
                      className={`rounded-xl border p-4 ${alreadyAdded ? "border-green-500/30 bg-green-500/5" : "border-white/10 bg-white/5"}`}
                    >
                      <div className="flex items-center justify-between">
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const searchRes = await fetch(
                                  `/api/bmi-office?action=search&q=${lp.id}&max=5`,
                                );
                                const results = await searchRes.json();
                                let age: number | null = null;
                                if (Array.isArray(results) && results.length > 0) {
                                  const detailRes = await fetch(
                                    `/api/bmi-office?action=person&id=${results[0].localId}`,
                                  );
                                  const p = await detailRes.json();
                                  if (p.birthDate) {
                                    age = Math.floor(
                ...
                return (
                  <div className="space-y-3">
                    <div className="rounded-lg border border-[#8652FF]/30 bg-[#8652FF]/5 p-3 text-center">
                      ...
                      {linkedSelected.age !== null && (
                        <p className="text-white/40 text-xs">Age: {linkedSelected.age}</p>
                      )}
                    </div>
                    <p className="text-white/50 text-xs">Select racer type:</p>
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        onClick={() => {
                          handleAddLinkedRacer(linkedSelected.id, "adult");
                          setLinkedSelected(null);
                        }}
                      ...
                      <button
                        onClick={() => {
                          handleAddLinkedRacer(linkedSelected.id, "junior");
                          setLinkedSelected(null);
                        }}
                    ...
                    <button
                      onClick={() => setLinkedSelected(null)}
                      className="text-white/30 text-xs hover:text-white/50 transition-colors block mx-auto"
                    >
                      ← Back to list
                    </button>
                  </div>
