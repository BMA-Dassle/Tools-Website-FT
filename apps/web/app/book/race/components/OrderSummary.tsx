...
type BookingState =
  ...
  | {
      ...
      savedCards?: import("@/components/square/SavedCardSelector").SavedCard[];
    }
  | { status: "confirmed" }
...
export default function OrderSummary({
  ...
  async function runBookingFlow() {
    trackBookingReview();
    console.log(
      "[runBookingFlow] STARTED — bills:",
      JSON.stringify(
        bills.map((b) => ({ billId: b.billId, personId: b.personId, racer: b.racerName })),
      ),
    );
      ...
      for (const bill of bills) {
        try {
          const overviewRes = await fetch(
            `/api/sms?endpoint=bill%2Foverview&billId=${bill.billId}${ck ? `&clientKey=${ck}` : ""}`,
          );
          ...
          if (overview.lines) {
            ...
            for (const l of overview.lines as any[]) {
              ...
              if (l.kind === 3 && String(l.productId) === "11253570") {
                const memPrice =
                  l.totalPrice?.find((p: { depositKind: number }) => p.depositKind === 0)?.amount ??
                  0;
                ...
                console.log(
                  "[bill overview] skipping BMI auto-added membership:",
                  l.name,
                  "subtracted:",
                  memPrice + memTax,
                );
              ...
              const cashPrice = l.totalPrice?.find(
                (p: { depositKind: number }) => p.depositKind === 0,
              );
              ...
              bmiLines.push({
                ...
                racers: lineRacers.length > 0 ? lineRacers : undefined,
                time: lineTime || undefined,
  ...
  async function recordClickwrap(
    orderId: string,
    cashOwed: number,
    cardLast4?: string,
    cardBrand?: string,
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      const racersToRegister =
        verifiedRacers.length > 0
          ? verifiedRacers
          : bills
              .filter((b) => b.personId)
              .map((b) => ({ personId: b.personId!, fullName: b.racerName }));
      for (const racer of racersToRegister) {
        if (racer.personId) {
          try {
            ...
            const rawJson =
              `{"personId":${racer.personId},"orderId":${primaryBillId},` + regBody.slice(1);
      ...
      const raceItems = state.bmiLines
        ...
        .map(
          (l) =>
            `${l.name}${l.quantity > 1 ? ` x${l.quantity}` : ""}${l.time ? ` @ ${formatTime(l.time)}` : ""}`,
        )
        .join(", ");
      ...
      for (const bill of bills) {
        try {
          ...
          const ovRes = await fetch(
            `/api/sms?endpoint=bill%2Foverview&billId=${bill.billId}${ovCk ? `&clientKey=${ovCk}` : ""}`,
          );
          const ov = await ovRes.json();
          billOverviews.push({
            ...ov,
            _racerName: bill.racerName,
            _personId: bill.personId,
            _billId: bill.billId,
          });
        } catch {
          /* skip */
      ...
      console.log(
        ...
        bookings.length,
        "racerNames:",
        bookings.map((b) => b.racerNames),
        "verifiedRacers:",
        verifiedRacers.length,
        verifiedRacers.map((r) => r.fullName),
      );
      ...
      if (racerAssignments.length === 0) {
        try {
          ...
          if (stored) {
            ...
            console.log(
              "[booking record] loaded racerAssignments from sessionStorage:",
              racerAssignments.length,
            );
          } else {
      ...
      console.log(
        "[booking record] racerAssignments:",
        racerAssignments.length,
        "primaryPersonId:",
        personId || sessionStorage.getItem("primaryPersonId") || "none",
      );
        ...
        packageHeats: bookings.some((b) => b.packageId)
          ? bookings
              ...
              .map((b) => {
                ...
                const ref =
                  pkgDef?.races.find((r) =>
                    r.tracks.some((t) => t.productId === String(b.product.productId)),
                  )?.ref ?? null;
                return {
                  packageId: b.packageId ?? null,
                  ref,
                  productId: String(b.product.productId),
                  sessionId: b.proposal?.blocks?.[0]?.block
                    ? ((b.proposal.blocks[0].block as { sessionId?: string | number }).sessionId ??
                      null)
                    : null,
                  start: b.block.start,
                  stop: b.block.stop,
      ...
      if (isCreditOrder) {
        ...
        window.location.href = `${confirmationPath}?billId=${orderId}&billIds=${allBillIds.join(",")}&racerNames=${bills.map((b) => encodeURIComponent(b.racerName)).join(",")}&personIds=${bills
          .filter((b) => b.personId)
          .map((b) => b.personId)
          .join(",")}`;
        return;
      ...
      if (isVerifiedReturningRacer) {
        ...
        try {
          ...
          /* non-fatal — proceed without saved cards */
        }
      }
  ...
  if (state.status === "paying") {
    const confirmUrl = `${confirmationPath}?billId=${state.orderId}&billIds=${state.allBillIds.join(",")}&racerNames=${bills.map((b) => encodeURIComponent(b.racerName)).join(",")}&personIds=${bills
      .filter((b) => b.personId)
      .map((b) => b.personId)
      .join(",")}`;
      ...
      <PaymentForm
        ...
        onSuccess={(result) => {
          ...
          void recordClickwrap(
            state.orderId,
            state.cashOwed,
            result.cardLast4 ?? undefined,
            result.cardBrand ?? undefined,
          );
        ...
        onCancel={() =>
          setState({
            ...
            cashOwed: state.cashOwed,
            creditApplied: 0,
            bmiTotal: state.cashOwed,
            bmiSubtotal: state.cashOwed,
            bmiTax: 0,
            bmiLines: [],
          })
        }
    ...
    <div className="space-y-6 max-w-lg mx-auto">
      ...
      <div className="text-center">
        <h2 className="text-2xl font-display text-white uppercase tracking-widest mb-2">
          {isBooked
            ? state.status === "booked" && state.isCreditOrder
              ? "Review & Confirm"
              : "Review & Pay"
            : "Preparing Order..."}
        </h2>
        {isBooked && (
          <p className="text-white/50 text-sm">
            Your heat{bookings.length > 1 ? "s are" : " is"} reserved. Complete your booking below.
          </p>
        )}
      </div>
              ...
              {cards.map((card) => {
                if (card.type === "package") {
                  ...
                  const cardPkg =
                    ...
                    (selectedPackage && selectedPackage.id === card.packageId
                      ? selectedPackage
                      : null);
                  ...
                  const isFallback =
                    ownBookings.length === 0 && selectedPackage?.id === card.packageId;
                  ...
                  const displayName =
                    cardPkg.category === "junior" &&
                    bookings.some(
                      (b) =>
                        b.packageId !== card.packageId &&
                        getPackageIgnoreFlag(b.packageId)?.name === cardPkg.name,
                    )
                      ? `${cardPkg.name} (Junior)`
                      : cardPkg.name;
                  ...
                  return (
                    <div
                      key={`package-${card.packageId}`}
                      className="px-4 py-4 bg-amber-500/[0.06]"
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        {onRemovePackage && state.status === "booked" && (
                          <button
                            ...
                            onClick={() => {
                              void onRemovePackage(card.packageId);
                            }}
                            className="text-red-400/40 hover:text-red-400 transition-colors p-0.5 -mr-1 shrink-0"
                          >
                            <svg
                      ...
                      {heatLabels && <p className="text-white/50 text-[11px] mb-2">{heatLabels}</p>}
                ...
                if (card.type === "race") {
                  ...
                  return (
                    ...
                    >
                      ...
                      {onRemoveBooking &&
                        (bookings.length > 1 || b.quantity > 1) &&
                        state.status === "booked" &&
                        xBtn(() => onRemoveBooking(card.bookingIdx))}
                    </div>
                  );
                }
                ...
                if (card.type === "pov") {
                    ...
                    return (
                      <div key="pov" className="px-4 py-4 bg-amber-500/[0.06]">
                        ...
                        <ul className="space-y-1 text-xs text-white/70 mb-2">
                          <li className="flex items-baseline justify-between gap-2">
                            <span>
                              <span className="text-emerald-400">✓</span> Racing License × {racers}
                            </span>
                          ...
                          <li className="flex items-baseline justify-between gap-2">
                            <span>
                              <span className="text-emerald-400">✓</span> POV Race Video × {racers}
                            </span>
                            <span className="text-white/50">
                  ...
                  return (
                    <div key="pov" className="px-4 py-3 flex items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-[#00E2E5] text-xs font-bold uppercase tracking-wider shrink-0">
                            Add-On
                          </span>
                          <span className="text-white font-semibold text-sm truncate">
                            POV Video
                        ...
                        <p className="text-[#00E2E5]/60 text-xs mt-0.5">
                          ${(pov!.price * pov!.quantity).toFixed(2)}
                        </p>
                      </div>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {a.quantity > 1 && (
                          <span className="text-white/20 text-xs shrink-0">x{a.quantity}</span>
                        )}
                      </div>
                      <p className="text-white/40 text-xs mt-0.5">
                        {a.selectedTime
                          ? formatTime(a.selectedTime)
                          : a.perPerson
                            ? `${a.quantity} ${a.quantity === 1 ? "person" : "people"}`
                            : ""}
                        <span className="text-[#00E2E5]/60 ml-2">
                    ...
                    {onRemoveAddOn &&
                      state.status === "booked" &&
                      xBtn(() => onRemoveAddOn(card.addOnIdx))}
                  </div>
                );
              })}
            </div>
                ...
                <div key={i}>
                  <div className="flex justify-between text-sm items-center">
                    <div className="text-white/60 flex-1">
                      ...
                      {line.time && (
                        <span className="text-white/30 ml-1">{formatTime(line.time)}</span>
                      )}
                    </div>
                      ...
                      {canRemove && (
                        <button
                          ...
                          onClick={async () => {
                            try {
                              await fetch(
                                `/api/bmi?endpoint=booking%2FremoveItem${getBookingClientKey() ? `&clientKey=${getBookingClientKey()}` : ""}`,
                                {
                                  method: "POST",
                                  headers: { "content-type": "application/json" },
                  ...
                  {isLicense && (
                    <p className="text-white/30 text-xs mt-0.5 ml-1">
                      One-year license includes use of head sock, helmet, and access to the FastTrax
                      app for race scheduling.
                    </p>
                  )}
                </div>
              );
                ...
                <>
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-green-400">
                      -{state.creditApplied} credit{state.creditApplied !== 1 ? "s" : ""}
                    </span>
                  </div>
                  <div className="border-t border-white/10 pt-2 flex justify-between font-bold">
                    <span className="text-white">
                      {state.cashOwed > 0 ? "Due Now" : "Amount Due"}
                    </span>
                    <span
                      className={`text-lg ${state.cashOwed > 0 ? "text-[#00E2E5]" : "text-green-400"}`}
                    >
      ...
      {isBooked &&
        (() => {
          const hasRacing =
            state.status === "booked" && state.bmiLines.some((l) => l.productGroup === "Karting");
          const hasLicense =
            state.status === "booked" &&
            state.bmiLines.some((l) => l.name.toLowerCase().includes("license"));
          return (
            <>
                  ...
                  {isPaying ? (
                    ...
                    <>
                      {state.status === "booked" && state.isCreditOrder
                        ...
                        : `Pay $${(state.status === "booked" ? state.cashOwed : total).toFixed(2)} →`}
                    </>
                  )}
                </button>
              </div>
              <div className="text-center">
                <button
                  onClick={() => {
                    if (
                      !confirm(
                        "Cancel your entire booking? This will remove all items including any attraction reservations.",
                      )
                    )
                      return;
                    // Cancel the bill
                    fetch(
                      `/api/bmi?endpoint=bill/${billId}/cancel${getBookingClientKey() ? `&clientKey=${getBookingClientKey()}` : ""}`,
                      { method: "DELETE" },
                    ).catch(() => {});
