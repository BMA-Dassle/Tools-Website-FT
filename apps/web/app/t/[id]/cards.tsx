...
export function PreRaceCard({
  ...
  const startsInText =
    mins <= 0
      ...
      : mins < 60
        ? `Check-in closes in ${mins} min`
        : `Check-in closes in ${Math.floor(mins / 60)}h ${mins % 60}m`;
...
export function InvalidCard({ details, pending }: { details?: CardDetails; pending?: boolean }) {
  const trackColor = details ? trackColorFor(details) : "#9ca3af";
  // "pending" = a confirmed booking whose session assignment hasn't landed yet
  // (never rostered) — NOT removed. Reassure instead of alarming (owner 2026-07-19).
  ...
  return (
    <div className="rounded-2xl border border-white/15 bg-white/[0.04] p-6 sm:p-8 text-center">
      ...
      <p className="text-white/50 text-sm leading-relaxed max-w-xs mx-auto">
        {pending
          ? "Your race is confirmed — your e-ticket will fill in with your check-in details shortly. Check back in a few minutes."
