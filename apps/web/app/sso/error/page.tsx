/**
 * The one page a staff member sees when sign-in does not end on a board.
 *
 * It exists to make the next step obvious and the support call short: what went
 * wrong in plain words, the stable code, and — when the gateway gave us one —
 * the request id that finds this exact attempt in the gateway's audit log. A
 * generic "Authentication error" costs someone twenty minutes.
 *
 * Self-contained inline styles on purpose: this renders on both brand hosts and
 * on the admin host, before any session exists, and must never depend on the
 * storefront chrome it is deliberately routed around.
 */
...
const MESSAGES: Record<string, { title: string; body: string }> = {
  ...
  SSO_E_CONFIG: {
    ...
    body: "This is a deploy problem, not your account. Send Eric the code below — the SSO environment block is missing or incomplete.",
  },
};
