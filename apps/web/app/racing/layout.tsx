...
/** "Mon-Thu 3-11 PM, Fri 3 PM-12 AM, Sat 11 AM-12 AM, Sun 11 AM-11 PM" — built
 *  from the hours registry so the SEO copy can't drift from the nav/footer. */
function hoursSentence(): string {
  const today = etDateIso();
  return fasttraxHoursGroups(today)
    .map(
      (g) =>
        `${formatHoursGroupLabel(g)} ${formatHoursRangeTerse(fasttraxHoursFor(g.weekdays[0], today))}`,
    )
    .join(", ");
}

function racingFaqList() {
  return [
    {
    ...
    },
    {
      ...
      answer:
        "All racers start in Starter tier. To unlock Adult Intermediate, you need a lap time of 41s on the Blue Track, 46s on the Red Track, or 1:28 on the Mega Track. For Adult Pro, you need 32.5s (Blue), 37s (Red), or 1:08.5 (Mega) in Intermediate. Junior Intermediate requires a 1:15 lap in Junior Starter, and Junior Pro requires a 45s lap in Junior Intermediate. You cannot skip levels.",
    },
    {
    ...
    },
    {
      ...
      answer: `FastTrax is Fort Myers' top rainy day activity — 63,000 sq ft of fully indoor, climate-controlled entertainment including go-kart racing on multi-level tracks, 50+ arcade games, duckpin bowling, shuffleboard, and Nemo's Trackside. No weather worries. Open ${hoursSentence()}.`,
    },
    {
