...
export default function KbfAdminClient({ token }: { token: string }) {
  ...
  const [futureReservations, setFutureReservations] = useState<
    { passId: number; reservationId: number; bookedAt: string }[]
  >([]);
  ...
  const doSearch = useCallback(async () => {
    ...
    try {
      ...
      const futureRez: { passId: number; reservationId: number; bookedAt: string }[] =
  ...
  function selectPass(pass: KbfPass, redeemed: RedeemedPair[]) {
    ...
    for (const m of pass.members) {
      ...
      const isRedeemed = redeemed.some((r) => r.passId === m.passId && r.slot === m.slot);
      selections.push({
      ...
      const res = await fetch("/api/admin/kbf/hold", {
        ...
        body: JSON.stringify({
          ...
          bowlers: selected.map((b) => ({
            ...
            kbfRelation:
              b.relation === "kid" ? "kid" : b.relation === "family" ? "family" : undefined,
            shoeSize: b.shoeSize,
  ...
  async function handleBowlNowConfirm() {
    ...
    try {
      ...
      if (data.ok) {
        setProgress((prev) => [
          ...prev,
          `Lane ${holdLaneNumber} confirmed`,
          "Lane opened!",
          "Shoes sent to KDS!",
          `Lane ${holdLaneNumber} is live!`,
        ]);
  ...
  async function handleBookLaneConfirm() {
    ...
    try {
      ...
      if (data.ok) {
        setProgress((prev) => [
          ...prev,
          `Booked for ${selectedDate} at ${formatHourMinute(selectedHour!, selectedMinute!)}!`,
        ]);
  ...
  const availableMinutes =
    selectedHour !== null
      ? [
          ...new Set(availableSlots.filter((s) => s.hour === selectedHour).map((s) => s.minute)),
        ].sort((a, b) => a - b)
      : [];

  const isHolding = phase === "hold" && mode === "book-lane" && holdQamfId;
  const canConfirmBookLane =
    isHolding && selectedDate && selectedHour !== null && selectedMinute !== null;
