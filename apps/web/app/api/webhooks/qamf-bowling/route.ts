...
async function processEvent(
  ...
  if (eventType === "reservation.updated") {
    ...
    const webhookBookedAt = data?.BookedAt as string | undefined;
    if (webhookBookedAt && reservation.bookedAt) {
      ...
      if (!isNaN(incomingMs) && Math.abs(incomingMs - storedMs) > 60_000) {
        try {
          ...
          console.log(
            `[qamf-bowling] synced BookedAt neonId=${reservation.id} from=${new Date(storedMs).toISOString()} to=${webhookBookedAt}`,
          );
        } catch (err) {
          console.warn(
            `[qamf-bowling] BookedAt sync failed neonId=${reservation.id}:`,
            err instanceof Error ? err.message : err,
          );
        }
      }
