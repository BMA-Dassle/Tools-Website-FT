 ...
 * Body: { shortId }
 *
 * Called after the guest signs and pays. Generates a tamper-evident PDF
 * with the document seal, agreements, signature, and full audit trail.
 * Emails it from the planner (CC'd to planner).
 */
...
export async function POST(req: NextRequest) {
  ...
  // Generate PDF
  const pdfBytes = await generateContractPdf(quote, auditLog);
  ...
  // Store URL in Neon
  const q = sql();
  await q`UPDATE group_function_quotes SET signed_pdf_url = ${blob.url}, updated_at = NOW() WHERE id = ${quote.id}`;
  ...
  // Email the PDF to guest from planner (CC planner)
...
<div style="max-width:600px;margin:0 auto">
  ...
  <div style="background:#1e293b;padding:24px;border:1px solid rgba(148,163,184,0.1)">
    ...
    <div style="text-align:center;margin:24px 0">
      <a href="${blob.url}" style="display:inline-block;padding:12px 32px;background:linear-gradient(135deg,#06b6d4,#2563eb);color:white;text-decoration:none;border-radius:999px;font-weight:700;font-size:14px">Download Signed Contract</a>
    </div>
  </div>
