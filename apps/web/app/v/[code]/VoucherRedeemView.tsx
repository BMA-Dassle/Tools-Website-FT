...
export function VoucherRedeemView({ status }: { status: VoucherStatus }) {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md flex-col px-5 py-10">
      ...
      <div className="mt-3 rounded-2xl bg-neutral-100 px-5 py-4 font-mono text-2xl tracking-[0.12em] text-neutral-900">
      ...
      <ul className="mt-6 space-y-2">
        {status.items.map((i) => (
          <li key={i.index} className="flex items-center justify-between gap-3 text-lg">
            <span className={i.spent ? "text-neutral-400 line-through" : "text-neutral-900"}>
              {i.label}
            </span>
      ...
      {done ? (
        <div className="mt-8 rounded-2xl border-2 border-emerald-500/40 bg-emerald-50 p-5">
        ...
        <p className="mt-8 text-lg text-neutral-700">{REASON_COPY.voided}</p>
      ) : expired ? (
        <p className="mt-8 text-lg text-neutral-700">{REASON_COPY.expired}</p>
      ) : redeemable.length === 0 ? (
        <div className="mt-8">
        ...
        <>
          <h1 className="mt-8 text-2xl font-extrabold text-neutral-900">
            Load it on your card
          </h1>
          <p className="mt-1 text-neutral-600">
            Enter the number printed on the back of your game card.
          </p>

          <label className="mt-5 block text-sm font-semibold text-neutral-700" htmlFor="v-acct">
            Card number
          </label>
          <input
            id="v-acct"
            ...
            placeholder="1063464"
            className="mt-1 w-full rounded-xl border-2 border-neutral-300 px-4 py-3 font-mono text-xl text-neutral-900 focus:border-neutral-900 focus:outline-none"
          />

          <label className="mt-4 block text-sm font-semibold text-neutral-700" htmlFor="v-loc">
            Where do you play?
          </label>
          <select
            id="v-loc"
            ...
            className="mt-1 w-full rounded-xl border-2 border-neutral-300 bg-white px-4 py-3 text-lg text-neutral-900 focus:border-neutral-900 focus:outline-none"
          >
            {CENTER_LIST.map((c) => (
          ...
          <div className="mt-3 min-h-[24px] text-sm text-red-600" role="alert" aria-live="polite">
            {error ?? ""}
          </div>
          ...
          <button
            ...
            className="mt-2 w-full rounded-full bg-neutral-900 px-6 py-4 text-lg font-bold text-white disabled:opacity-40"
          >
