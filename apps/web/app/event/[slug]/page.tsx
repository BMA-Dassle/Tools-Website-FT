...
type Step =
  | "gate"
  | "signup" // dedicated RSVP screen: pick center → enter email
...
function includedIcon(item: string): TablerIcon {
  ...
  if (s.includes("drink")) return IconGlassCocktail;
  if (s.includes("buffet") || s.includes("food")) return IconToolsKitchen2;
  if (s.includes("bowl")) return IconBowling;
  if (s.includes("race") || s.includes("kart")) return IconCar;
  if (s.includes("gift")) return IconGift;
  return IconChristmasTree;
}
  ...
  return (
    <div className="relative min-h-screen bg-[#000418] pt-[140px]" style={accentStyle}>
      ...
      <div className="relative z-10">
        ...
        {/* ═══ LANDING — marketing (gate) + dedicated RSVP screen (signup) ═══ */}
        {landing && (step === "gate" || step === "signup") && (
          <div>
            {step === "gate" && (
              <>
                {/* ── Hero ── */}
                      ...
                      {landing.locations && landing.locations.length > 0
                        ? selectedLocation
                          ...
                          : landing.locations
                              ...
                              .join("  ·  ") +
                            (landing.eventTime ? `  ·  ${landing.eventTime}` : "")
                ...
                {/* ── Live countdown band — both events ── */}
                {landing.countdown && landing.locations && landing.locations.length > 0 && (
                  <div className="mx-auto max-w-5xl px-4 pt-10">
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      {landing.locations.map((loc) => (
                        <div
                          key={loc.key}
                          className="flex items-center justify-between rounded-2xl border border-(--accent)/25 bg-white/5 px-5 py-4 backdrop-blur-sm"
                        >
                          <div>
                  ...
                  <div className="mx-auto max-w-5xl px-4 pt-14">
                    <div className="relative overflow-hidden rounded-3xl border border-(--accent)/25">
                      ...
                      <div className="relative p-6 md:p-10">
                        ...
                        <p className="mb-8 mt-1 flex items-center justify-center gap-1.5 text-center text-sm text-white/60">
                          <IconGift className="h-4 w-4 text-(--accent)" stroke={1.5} />
                          Our gift to every guest
                        </p>
                        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                          {landing.included.map((inc) => {
                            const Icon = includedIcon(inc.item);
                            return (
                              <div
                                key={inc.item}
                ...
                {/* ── Closing CTA → dedicated RSVP screen ── */}
                <div className="px-4 pb-20 pt-4 text-center">
                  <button
                    type="button"
                    onClick={() => setStep("signup")}
                    className="rounded-xl bg-(--accent) px-10 py-4 text-base font-bold text-(--accent-fg) shadow-lg transition-colors hover:bg-(--accent-hover)"
                  >
                    {landing.ctaLabel ?? "Sign Up"}
                  </button>
                </div>
            ...
            {/* ── Dedicated RSVP screen: choose center → email ── */}
            {step === "signup" && (
                          ...
                          {landing.locations.map((loc) => (
                            ...
                            >
                              ...
                              {landing.countdown && countdownStr(loc.date) && (
                                <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-(--accent)/15 px-3 py-1 text-xs font-bold text-(--accent)">
              ...
              <div className="max-w-md mx-auto">
                <div className="rounded-2xl border border-white/10 bg-white/3 p-8">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <p className="flex items-center gap-2 font-display text-base uppercase tracking-widest text-white">
                        <IconCar className="h-5 w-5 text-(--accent)" stroke={1.5} />
                        Attending + Racing
                      </p>
                      <p className="mt-1 text-sm text-white/55">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/3 p-8 text-center">
                  ...
                  <form onSubmit={handleAttendSubmit} className="space-y-4">
                    ...
                    <div className="text-left">
                      ...
                      >
                        <option value="0" style={{ backgroundColor: "#000418", color: "#fff" }}>
                          Just me
                        </option>
            ...
            {step === "dashboard" && guest && (
              <div className="space-y-8">
                  ...
                  {hasFreeflow ? (
                    <>
                      <p>
                        ...
                        all require a signed waiver and a pre-booked time slot. You can only book
                        for yourself, but your name will appear on the booking so your coworkers can
                        see who&rsquo;s in each session.
                      </p>
                      <p>
                        All other activities are <strong className="text-white">free-flow</strong>{" "}
                        and available at your leisure throughout the event.
                      </p>
                    </>
                  ) : (
                    <p>
                      <strong className="text-white">Go-Kart Racing</strong> requires a signed
                      waiver and a pre-booked time slot. You can only book for yourself, but your
                      name will appear on the heat so others can see who&rsquo;s racing.
                    </p>
                  )}
                  {event.mealWindow && (
                ...
                <div className="rounded-xl border border-white/8 bg-white/3 p-4 text-xs text-white/40 space-y-1">
                  ...
                  <p>
                    &middot; Please arrive{" "}
                    <strong className="text-white/60">15 minutes early</strong> for racing check-in.
                  </p>
                </div>
              </div>
            )}
            ...
            {step === "confirmation" && guest && (
              <div className="max-w-md mx-auto space-y-6">
                  ...
                  <div className="rounded-xl border border-(--accent)/20 bg-(--accent)/5 p-4 text-center text-sm">
                    ...
                    {attendInfo && (
                      <p className="mt-2 text-white/50">
                        ...
                        {attendInfo.guests === 0
                          ? "Just you"
                          : `You +${attendInfo.guests} guest${attendInfo.guests === 1 ? "" : "s"}`}
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="rounded-xl border border-white/8 bg-white/3 p-4 text-xs text-white/40 space-y-1">
                  <p>
                    &middot; Please arrive{" "}
                    <strong className="text-white/60">15 minutes early</strong> for check-in.
                  </p>
