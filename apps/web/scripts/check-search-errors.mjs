...
for (const site of sites) {
  ...
  try {
    ...
    if (sitemaps.sitemap?.length) {
      ...
      for (const sm of sitemaps.sitemap) {
        ...
        console.log(
          `  ${status} ${sm.path}  indexed:${indexed}/${submitted}  errors:${errors}  warnings:${warnings}`,
        );
      }
    }
  } catch (e) {
    console.log("  Sitemaps: " + e.message);
  }
  ...
  try {
    ...
    if (enh.rows?.length) {
      ...
      for (const r of enh.rows) {
        console.log(
          `  ${r.keys[0].padEnd(35)} clicks:${String(r.clicks).padStart(5)}  impressions:${String(r.impressions).padStart(6)}  pos:${r.position.toFixed(1)}`,
        );
      }
    } else {
    ...
    console.log("  Appearances: " + e.message);
  }
  ...
  const urls =
    site.label === "FastTrax"
      ? [
          ...
          "https://fasttraxent.com/pricing",
          "https://fasttraxent.com/group-events",
          "https://fasttraxent.com/racing",
        ]
      : [
          "https://headpinz.com/",
  ...
  for (const url of urls) {
    try {
      ...
      if (!r) {
        console.log(`  ❓ ${url}: ${JSON.stringify(result).substring(0, 100)}`);
        continue;
      }
      const verdict = r.indexStatusResult?.verdict || "UNKNOWN";
      const crawled = r.indexStatusResult?.lastCrawlTime
        ? new Date(r.indexStatusResult.lastCrawlTime).toLocaleDateString()
        : "never";
      const coverage = r.indexStatusResult?.coverageState || "";
      const mobile = r.mobileUsabilityResult?.verdict || "?";
      const richRes =
        r.richResultsResult?.detectedItems?.map((i) => i.richResultType).join(", ") || "none";
      const richIssues =
        r.richResultsResult?.detectedItems?.flatMap(
          (i) =>
            i.items?.flatMap(
              (ii) => ii.issues?.map((iss) => `${iss.severity}: ${iss.issueMessage}`) || [],
            ) || [],
        ) || [];
      const icon = verdict === "PASS" ? "✅" : verdict === "NEUTRAL" ? "🟡" : "❌";
      console.log(
        `  ${icon} ${url.replace("https://fasttraxent.com", "").replace("https://headpinz.com", "") || "/"}`,
      );
      console.log(
        `       verdict:${verdict}  coverage:${coverage}  crawled:${crawled}  mobile:${mobile}`,
      );
...
if (bingToken) {
  ...
  console.log(`${"═".repeat(60)}`);
  try {
    const bingSites = await fetch(
      "https://ssl.bing.com/webmaster/api.svc/json/GetUserSites?apikey=" + bingToken,
    ).then((r) => r.json());
