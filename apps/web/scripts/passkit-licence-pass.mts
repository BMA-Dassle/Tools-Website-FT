...
// .env.local is gitignored, so it lives only in the MAIN tree — walk up to find
// it. Most work here happens in nested worktrees, where a cwd-only lookup dies
// with a bare ENOENT that says nothing about why.
const SPLIT_RE = /\r?\n/;
function loadEnvLocal(): void {
  let dir = process.cwd();
  for (let i = 0; i < 8; i++) {
    for (const candidate of [resolve(dir, ".env.local"), resolve(dir, "apps", "web", ".env.local")]) {
      if (!existsSync(candidate)) continue;
      for (const l of readFileSync(candidate, "utf8").split(SPLIT_RE)) {
 ...
 */
/**
 * Empty by default. This used to hold a real-but-not-yours heat so the field
 * could be SEEN — and it then sat on a live pass showing yesterday's race,
 * because nothing rewrites it until the cron owns this field. A pass must never
 * assert a race we cannot stand behind.
 */
const NEXT_RACE_RAW = process.env.NEXT_RACE_RAW || "";
...
/** SMS-Timing client key — used by the APP's QR payload shape, not ours. */
const CLIENT_KEY = process.env.BMI_CLIENT_KEY || "headpinzftmyers";

/**
 * SMS-Timing site id in the authenticate URL (`/908/authenticate/`). Confirmed
 * from a real register QR 2026-08-04.
 */
const SMSTIM_SITE = process.env.SMSTIM_SITE || "908";

/** Resting value for the check-in field. Must match what the clear-down writes,
 *  or the two would fight and alert the racer on every hand-off. */
const CHECKIN_IDLE = "Not checking in yet";
...
const field = (o: {
  ...
  appleWalletFieldRenderOptions: {
    ...
    // ALWAYS SENT, even when empty. PassKit MERGES the template on PUT rather
    // than replacing it, so omitting this key leaves whatever was there before
    // — a changeMessage can only be REMOVED by explicitly blanking it.
...
const fmtNextRace = (raw: string, long = false): string => {
  ...
  if (!heatId) return "None in next 2 hrs";
  const dt = new Date(heatId);
  if (Number.isNaN(dt.getTime())) return "None in next 2 hrs";
...
async function main() {
  ...
  if (person) {
    console.log(
      `pandora: ${person.firstName} ${person.lastName}  waiver=${person.waiverExpiry ?? "—"}  lastVisit=${person.lastVisit ?? "—"}`,
    );
  } else {
  ...
  const meta = {
    ...
    checkinStatus: process.env.CHECKIN_STATUS || "Not checking in yet",
    licenceUrl: `https://headpinz.com/r/${CODE}`,
    // BARCODE PAYLOAD — the SMS-Timing AUTHENTICATE url. This is the exact
    // shape a real BMI register QR uses, confirmed from a live one 2026-08-04:
    //
    //   https://smstim.in/908/authenticate/?login_code=6pmyyfhg4397c
    //
    // It is NOT the app's QR shape (`smstim.in?["clientKey","code"]`), which is
    // what this pass carried first and what the register rejected. Two
    // different formats on the same host: the app payload is a JSON array with
    // a clientKey, this one is a real path with the code as a query param and
    // a numeric SITE id instead of a client key.
    memberQr: `https://smstim.in/${SMSTIM_SITE}/authenticate/?login_code=${CODE}`,
  ...
  const DATA_FIELDS = [
    ...
    field({
      ...
      // NO changeMessage, deliberately. This field moves for reasons the racer
      // did not cause: the e-ticket cron only looks 2 hours ahead, so a race
      // further out reads as empty until it comes into range. With an alert
      // attached that becomes a false "your next race: none", followed by a
      // SECOND alert when the real one appears. Next race is AMBIENT — correct
      // when you open the pass, never an interruption. Only check-in buzzes.
    }),
    field({
    ...
    // "CHECKING IN NOW" push. Its changeMessage is a bare "%@", so whatever we
    // write becomes the lock-screen alert verbatim — no "Your next race:"
    // prefix wrapping an already-complete sentence.
    //
    // A BACK field on purpose: it is empty most of the time and would leave a
    // dead slot on a face that is already full. Apple fires the notification
    // for any field carrying a changeMessage whose value changed, back fields
    // included.
    //
    // Pushes are FREE and unlimited, which is the whole reason this is viable —
    // heats run every ~10-12 minutes, so an SMS rail would be a real per-message
  ...
  // NO PER-RACER IMAGE. EVER.
  //
  // Images uploaded here become TEMPLATE images, and one template serves every
  // pass in the programme — so a racer's photo does not go on their own card,
  // it goes on EVERYBODY'S. That is exactly what happened on 2026-08-04: Alex
  // Trepasso has a BMI photo, issuing his licence put his face into the shared
  // template, and it shipped inside Eric's .pkpass too. Invisible only because
  // a storeCard renders a strip and ignores thumbnails — a latent privacy leak,
  // not a near miss.
  //
  // Anything racer-specific must be a text FIELD (per-member metaData), never
  // an image. If per-racer artwork is ever genuinely needed it requires one
  // template per racer, which is not a thing worth wanting.
  const imgIds = await pk("POST", "/images", { imageData });
  console.log(
    `images → ${imgIds.status} (logo 2072x660, icon 660x660, NO thumbnail by design)`,
  );
  ...
  const put = await pk("PUT", "/template", {
    ...
    imageIds: {
      ...
      // Always cleared. The stock template we dressed shipped one, and a
      // per-racer photo must never live here (see above) — so there is no
      // circumstance in which this template carries a thumbnail.
      thumbnail: "",
      ...((imgIds.body as any)?.strip ? { strip: (imgIds.body as any).strip } : {}),
    },
    barcode: {
      ...tpl.barcode,
      payload: "${meta.memberQr}",
      altText: "${meta.code}",
      format: "QR",
    },
    // A GENERIC Apple pass has NO strip slot — it would silently ignore the
    // racing photo. storeCard is the style that renders one.
    // BRANDING THAT SURVIVES THE STACKED WALLET VIEW.
    //
    // With several cards in Wallet only the HEADER ROW of each shows — the strip
    // carrying our FastTrax mark is below the fold, so in a stack this card was
    // unidentifiable. `logoText` renders as TEXT beside the logo in that row, so
    // it reads at any size and sidesteps PassKit squaring the logo image into a
    // stamp (upload 2072x660, get 660x660 back).
    //
    // passType STORE_CARD is also load-bearing: a GENERIC pass has no strip slot
    // and would silently drop the racing photo.
    appleWalletSettings: {
  ...
  // RECORD THE PASS IN NEON. Without this row the live crons cannot find the
