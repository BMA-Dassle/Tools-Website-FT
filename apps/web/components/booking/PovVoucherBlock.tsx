...
export default function PovVoucherBlock({
  codes,
  caption,
  variant = "fasttrax",
  cached = false,
  ...
  return (
    <div className={`rounded-2xl border ${palette.card} p-6 sm:p-8`}>
      <div className="flex items-baseline justify-between flex-wrap gap-2 mb-4">
        <h3 className="font-display text-white text-xl uppercase tracking-widest">
          ...
          <p className="text-white/60 text-xs leading-relaxed">
            {caption ?? (
              <>
                ...
                <strong className="text-white/80">email and text</strong> letting you know your
                video is ready. Use the codes below to redeem it.
              </>
            )}
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-4 mt-4">
        {codes.map((code, i) => (
          <div key={i} className={`bg-white/10 border ${palette.chip} rounded-lg px-5 py-3`}>
            <p
              className={`${palette.chipLabel} text-xs font-semibold uppercase tracking-wider mb-1`}
            >
              Code {i + 1}
            </p>
