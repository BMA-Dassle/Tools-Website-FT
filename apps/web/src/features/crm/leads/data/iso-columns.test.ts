...
describe("the emitted SQL", () => {
  it("crm_contacts reads created_at / updated_at through ISO()", async () => {
    await getContact("9");
    const [stmt] = selects(/FROM crm_contacts c WHERE c\.id/);
    expect(stmt).toBeDefined();
    expect(stmt!.text).toContain(`${ISO("c.created_at")} AS created_at`);
    expect(stmt!.text).toContain(`${ISO("c.updated_at")} AS updated_at`);
  ...
  it("crm_accounts reads created_at / updated_at / archived_at through ISO()", async () => {
    ...
    const [stmt] = selects(/FROM crm_accounts a WHERE a\.name_key/);
    expect(stmt).toBeDefined();
    for (const col of ["created_at", "updated_at", "archived_at"])
      expect(stmt!.text).toContain(`${ISO(`a.${col}`)} AS ${col}`);
