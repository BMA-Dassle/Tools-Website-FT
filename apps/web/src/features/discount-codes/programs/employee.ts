...
/**
 * Stamp (or clear) the employee perks on the party, PURELY. The one member
 * named by `employee.memberId` carries the stamp; every other member's stamp is
 * removed, so a stale stamp from a previous match can never survive a re-match.
 * Used by the client reducer (display) AND the server reconcile (charge), so
 * the two can never disagree about who the employee is.
 */
export function stampEmployeeOnParty<T extends { id: string; employeePerks?: EmployeePerksStamp }>(
  party: T[],
  employee: Pick<SessionEmployee, "userId" | "firstName" | "memberId"> | null | undefined,
): T[] {
  return party.map((m) => {
