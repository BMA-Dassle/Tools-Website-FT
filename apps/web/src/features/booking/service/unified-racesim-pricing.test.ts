 ...
 * flat $15.95 × racers (owner 2026-09-01 — no day-split), the shared Square
...
describe("unified pricing — racesim cart", () => {
  it("prices a session at $15.95/racer on the shared Square catalog id", () => {
    ...
    expect(sqLineItems[0].basePriceMoney?.amount).toBe(1595);
    expect(sqLineItems[0].name).toContain("Track A");
    expect(sqLineItems[0].name).toContain("3:00 PM");
    expect(pricedLines).toHaveLength(1);
    expect(totalPriceCents).toBe(3190);
  });

  it("one line PER SESSION across tracks — 2 sessions × 2 racers × $15.95", () => {
    ...
    expect(totalPriceCents).toBe(6380);
  });

  it("charges the SAME $15.95 on a weekend — the day-split is gone", () => {
    // 2026-08-29 is a Saturday, which used to bill $16. The pack prices are all
    // struck off a flat $15.95 single, so a day-split here would make every
  ...
  it("quote mirror carries the same money (single, 1 racer)", () => {
    ...
    expect(q.subtotalCents).toBe(1595);
