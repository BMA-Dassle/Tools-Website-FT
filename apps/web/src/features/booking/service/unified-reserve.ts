...
export function buildCombinedLineItems(session: BookingSession): {
  ...
  /** EMPLOYEE PERKS: the team members' free single races priced to $0 this pay week. */
  employeeFree: EmployeeFreeHeats;
  /** Cents each team member's 50% removed from THEIR own racing + attraction
   *  units, keyed by party member id — the per-employee ledger figure only. */
  employeeSavingsByMember: Map<string, number>;
  ...
  // EMPLOYEE PERKS: cents removed by each team member's 50% (their own racing
  // split lines + attraction units), per member — for the perk ledger, never a
  // charge input.
  const employeeSavingsByMember = new Map<string, number>();
  const addEmployeeSavings = (memberId: string, cents: number) => {
    if (cents <= 0) return;
    employeeSavingsByMember.set(memberId, (employeeSavingsByMember.get(memberId) ?? 0) + cents);
  };
  ...
  // EMPLOYEE PERKS — each verified team member's free SINGLE races this pay
  // week (owner 2026-09-13: 2 per Wed–Tue week, per person). After credits/
  // packs/vouchers (the allowance covers only heats that would otherwise be
  // paid in cash) and BEFORE BOGO. Each employee's `usedThisWeek` is what the
  // display priced with; unifiedReserveInner has already hard-failed if any
  // of their ledgers moved.
  const employeeFree: EmployeeFreeHeats = activeComboSpecial(session)
    ? { heats: new Set(), memberIds: [] }
    : computeEmployeeFreeHeats(
        session.items,
        session.party,
        coveredBeforeBogo,
  ...
  // EMPLOYEE PERKS: cents each team member's 50% removed from THEIR OWN racing
  // split lines — differenced from the same builder, per member (a member whose
  // 50% is the BMI membership's shows $0). For the ledger only.
  for (const [memberId, cents] of employeeRacingSavingsByMember(session, excludedHeats)) {
    addEmployeeSavings(memberId, cents);
  }
  ...
  // labels 2026-08-01 — so heats covered by different codes group separately.
  {
    ...
    const covered: Array<{
      ...
      // The team members' free races (weekly allowance) — tagged so the review
    ...
    for (const { set, kind, labelFor } of covered) {
      ...
      for (const g of groups.values()) {
        ...
        // attraction line.
        // EMPLOYEE-PERK-covered heats take the same $0 line (2026-09-14): a
        // returning team member booking only their two free single races on
        // the kiosk (no licence line, nothing else in the cart) built ZERO
        // Square lines — the 09-06 voucher failure again, one coverage kind
        // over. Credit / pack / BOGO heats are deliberately unchanged: a
        // credit-only $0 order takes the legacy rail, and those flows are
        // smoked as-is.
        if (kind === "voucher" || kind === "employee-perk") {
  ...
  for (const item of session.items) {
    ...
    // EMPLOYEE PERKS — the team members' OWN gel-blaster / laser-tag units at
    // their entitlement percent (Employee Pass 50%, either source). Kiosk lines
    // name their players; a web line gives exactly one own unit per employee.
    // Those units split off into their own line below, named like the racing
    // split lines.
    ...
    // Per-member attribution of the split (a voucher may have covered some of
    // the line first, so serve members in party order until empQty runs out).
    {
      let left = empQty;
      for (const b of emp.byMember) {
        const units = Math.min(b.units, left);
        if (units <= 0) break;
        left -= units;
        addEmployeeSavings(b.memberId, (unitCents - empUnitCents) * units);
      }
    }
    if (empQty > 0) {
      sqLineItems.push({
...
async function unifiedReserveInner(
  ...
  // `session.employees` and every `party[].employeePerks` stamp are CLIENT
  // claims. Strip them, verify each signed token, re-check each user is still
  // on the active 7shifts roster, re-stamp exactly those members — anyone who
  // fails prices as a plain guest, loudly. Then EACH employee's weekly
  // free-race allowance: the display priced with `usedThisWeek` as the server
  // last read it; if that person's ledger now leaves FEWER free heats (used
  // at another kiosk in between, or the pay week rolled), the reserve
  // HARD-FAILS rather than charge more than the screen showed. The terminal
  // rail's $25 drift backstop would otherwise let a $17.99 heat through as a
  // silent overcharge.
  {
    const emp = await applyEmployeeToSession(input.session);
    if (emp.dropped.length > 0) {
      console.error(
        `[unified-reserve] EMPLOYEE PERKS DROPPED AT CHARGE: ${emp.dropped.join(", ")} ` +
          `(bill ${input.session.bmiBillId ?? "n/a"}) — pricing those as guests`,
      );
    }
    input = { ...input, session: emp.session };
    const weekNow = payWeekKey();
    for (const e of emp.employees) {
      const usedNow = await readFreeRacesUsed(e.userId, weekNow).catch(() => null);
      if (usedNow == null) continue;
      const shownRemaining = freeRacesRemaining({ usedThisWeek: e.claimedUsedThisWeek });
      ...
      if (realRemaining < shownRemaining) {
        throw new EmployeePerksChangedError(
          `Free races this pay week changed for ${e.firstName}: ${shownRemaining} shown, ${realRemaining} left`,
        );
      }
    }
  }
  ...
  const gzPurchase =
    session.context?.kiosk && kioskGzCartEnabled()
      ? resolveCartPurchase(session.gameCardPurchase, {
          // EMPLOYEE PERKS: doubled token credit when a verified team member is
          // on the booking — decided from the server-re-derived stamps (step
          // 0a½), never a client flag. Any team member on the order doubles the
          // cart's cards (the cards are not per person).
  ...
  // Keyed on the day-of order id + heat (free races) or + member (percent
  // rows), so a retry never double-counts and several team members on one
  // order each get their own rows. The free-race rows are what next week's
  // allowance is read from, per 7shifts user; the percent rows are the audit
  // trail Square cannot give us (price-key mechanism). NEVER fail a captured
  // booking on this.
  {
    const employees = sessionEmployees(session).filter((e) => !!e.memberId);
    const byMember = new Map(employees.map((e) => [e.memberId as string, e]));
    if (byMember.size > 0 && employeeMembers(session.party).length > 0) {
      try {
        const source = input.session.context?.kiosk ? "kiosk" : "web";
        const rows: Parameters<typeof recordPerkRedemptions>[0] = [];
        for (const item of session.items) {
          if (item.kind !== "race") continue;
          for (const h of item.heats) {
            if (!employeeFree.heats.has(h)) continue;
            const emp = h.assignedTo ? byMember.get(h.assignedTo) : undefined;
            if (!emp) continue;
            rows.push({
              userId: emp.userId,
              perk: "free-race",
        ...
        for (const [memberId, cents] of employeeSavingsByMember) {
          const emp = byMember.get(memberId);
          if (!emp || cents <= 0) continue;
          rows.push({
            userId: emp.userId,
            perk: "percent-off",
            weekKey: emp.weekKey,
            ...
            source,
            detail: { percentOff: EMPLOYEE_PROGRAM.percentOff, memberId },
          });
        }
