...
export interface ConsolidateAccountsParams {
  ...
  /** One or more source cards whose ENTIRE balance moves onto the target. */
...
/**
 * Consolidate — move the ENTIRE balance (cash, bonus cash, tokens, bonus tokens,
 * points, time) of one or more SOURCE accounts onto a TARGET account, in ONE
 * atomic server-side transaction, via TPI_ConsolidateAccounts (cloud SOAP). The
 * server computes and moves every value field, so the caller never enumerates
 * amounts. This is why it can't drop a field the way a manual credit-then-clear
 * does (which reads only the fields it knows and then wipes the rest), and why
 * it's the money-safe way to combine cards: value never sits nowhere, and there
 * is NO separate, unguarded clear step.
 *
 * Returns the raw result code (0 success, -1 server exception, -2 MAC not
 * registered). Idempotent on tpiTransactionID (a duplicate id returns 0 without
 * re-applying) — persist a stable id BEFORE calling and reuse it on retry.
 *
 * ⚠️ ON AMBIGUITY: heed the TPI rollback caveat (Az 11906–11909) — a mid-call
 *    failure may not roll back cleanly. NEVER blind-retry with a NEW id; retry
 *    only with the SAME id (dedup-safe), and on a still-ambiguous outcome
 *    re-query both accounts (verifyAccount) rather than assuming it landed.
 *
 * ⚠️ UNVERIFIED SHAPE: the op name + envelope (element order, the sourceAccounts
 *    array item element `<string>` vs `<AccountNumber>`, whether the TPI context
 *    is flat as here or a nested `<TpiTransactionContext>`, and whether the
 *    source is left at zero-balance vs de-registered) come from the TPI spec
 *    (docs/intercard-tpi-api.yaml), modelled on the LIVE-verified
 *    TPICreditAccounts envelope. NOT yet confirmed against the live service.
 *    Dry-run on THROWAWAY cards and adjust from the response before enabling.
 */
export async function consolidateAccounts(
  ...
  if (sourceAccounts.length === 0) {
    throw new IntercardError(
      "NO_ACCOUNTS",
      "consolidateAccounts requires at least one source account",
    );
  }
  const mac = macForCenter(locationCode); // per-location registration
  const now = new Date();

  // Array-of-scalar convention mirrors MAC_ID's <string> wrapping; if a live
  // dry-run faults, the likely fix is the item element (<string> vs <AccountNumber>).
  const sourcesXml = sourceAccounts.map((a) => `<string>${xmlEscape(a)}</string>`).join("");
  ...
  const resp = await soapCall(INTERCARD_TPI_URL, "TPI_ConsolidateAccounts", inner, mac);
