 ...
 *
 * `inline` drops the bar (no margin, border, or background) and renders just the
 * label + clock, so KioskFlow can sit it in the signed-in row instead of stacking
 * a second band above it (owner 2026-08-04: "those two bars are still on top…
 * slip this down to one line"). The urgency colours ride the text in that mode.
 */
...
export const KioskHoldBar = forwardRef<
  ReservationHoldHandle,
  UseReservationHoldOptions & { inline?: boolean }
>(function KioskHoldBar(props, ref) {
  ...
  if (inline) {
    return (
      <span className={`flex shrink-0 items-center gap-[12px] text-[22px] ${inlineTone}`}>
        <svg
        ...
        {isExpired ? (
          <button
            ...
            disabled={refreshing}
            className="k-tap font-bold underline"
          >
            {refreshing ? "Refreshing…" : "Expired — tap to refresh"}
          </button>
        ) : (
          <>
            <span className="k-num font-bold">{display}</span>
            {isWarn && (
              <button
                type="button"
                onClick={() => void refresh()}
                disabled={refreshing}
                className="k-tap rounded-full border border-current px-[18px] py-[4px] text-[18px] font-bold uppercase"
              >
                {refreshing ? "…" : "Extend"}
              </button>
            )}
