...
export interface ReceiptState {
  ...
  cardCodes: number;
  /** Whether THIS kiosk has a working card dispenser (capability "full"). */
  canDispense: boolean;
...
export type ReceiptPrimary =
  /** Dispense now — cards are the only value here. */
  | "print"
  /** Dispense now, then return to the order (cart legs exist). */
  | "print-continue"
  /** Nothing printable HERE (no dispenser) and nothing on an order — leave. */
  | "done"
...
export function receiptPlan(s: ReceiptState): ReceiptPlan {
  const printable = s.canDispense && s.cardCodes > 0;
  if (printable) {
    return {
      primary: s.cartVouchers > 0 || s.promoApplied ? "print-continue" : "print",
      // Leaving printable cards behind is the ONE silent-loss path — warn.
      warnOnBack: true,
    };
  }
  // No dispenser (or no cards): nothing this screen itself fulfils, so Back
  // never needs a warning — value either rides the order or is collected
