...
const BIN_BY_BYTE: Record<number, ErrorBinLevel> = {
  0x30: "ok", // empty
  0x31: "full",
  // This HB-HDN unit reports a FULL reject bin as st2=0x32 ('2'), not the doc's
  // 0x31 — without this row it decoded to "unknown" and the admin/hold treated a
  // full bin as unknown (owner-confirmed 2026-07-19: ok = empty, "unknown" = full).
  0x32: "full",
};
 ...
 * 8th raw; tolerate any actual length. (Bin-full state comes from the status
 * block's st2 byte, decoded by parseStatus — not from here.)
 */
