...
export type DemoMode = "event" | "vip" | "race" | "briefing" | "briefing-quals" | "off";
...
export function applyDemo(feed: TvFeed | null, mode: DemoMode, nowMs: number): TvFeed | null {
  ...
  if (mode === "briefing" || mode === "briefing-quals") {
    return { ...feed, briefing: demoBriefingSection(feed) };
  }
  if (mode === "race") {
 ...
 * helmet board, then quals, on the real schedule. `briefing-quals` skips straight
...
export function demoBriefingRooms(
   ...
   *  the admin page, so a `?demo=briefing-quals` typed into a tab would silently
