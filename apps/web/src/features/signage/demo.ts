...
export type DemoMode =
  ...
  | "briefing"
  | "briefing-return"
  /** The RETIRED 2026-08-11 welcome-back layout (who qualified / who didn't),
   *  preserved for review — "save the qualifiers page case we ever want it
   *  back" (owner 2026-08-23). Same fabricated return as briefing-return; the
   *  scene picks the old board instead of the exit board. */
  | "briefing-return-quals"
  | "results"
  | "results-none"
  | "results-pro"
  | "results-mega"
...
export function applyDemo(feed: TvFeed | null, mode: DemoMode, nowMs: number): TvFeed | null {
  ...
  if (mode === "briefing" || mode === "briefing-return" || mode === "briefing-return-quals") {
...
export function demoBriefingRooms(
  ...
  if (
    ...
    demoBriefingAnchor = {
      ...
      atMs: mode === "briefing" ? nowMs - 1_000 : nowMs - (videoMs + 31_000),
    };
  }
...
function demoBriefingSection(feed: TvFeed, mode: DemoMode): TvFeed["briefing"] {
  ...
  return {
    ...
    welcomeBack:
      mode === "briefing-return" || mode === "briefing-return-quals"
        ? {
            heatNumber: 58,
            raceType: "Starter",
            track: "red",
            ...
            // Arrival stamped NOW — motion mode's happy path — so a preview
            // still plays the greeting without waiting on a real camera.
            // Never a linger stamp: the nag is a one-shot aimed at a real
            // over-stayed room, and a preview that nags on open reads as a
            // bug.
