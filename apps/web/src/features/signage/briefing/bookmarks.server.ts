 ...
 *  Returns immediately — the write happens after the response. */
export function bookmarkBriefingStartAfter(
  args: Parameters<typeof bookmarkBriefingStart>[0],
): void {
  afterResponse(() => bookmarkBriefingStart(args));
}

/** "The room emptied." Same, for the send-to-holding press and the camera sweep. */
export function bookmarkBriefingEndAfter(args: Parameters<typeof bookmarkBriefingEnd>[0]): void {
  afterResponse(() => bookmarkBriefingEnd(args));
}
