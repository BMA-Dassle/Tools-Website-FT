...
/**
 * WHO BRIEFED HOW MANY GROUPS TODAY, off the log the board already polls.
 *
 * PURE, and deliberately over the log rather than over a fetch: the check-in
 * board holds every one of today's records in state to draw the Briefing log
 * panel, so the tally under its header is arithmetic on data already in the
 * page — no second endpoint, no second thing to be down. (The portal's TV asks
 * FastTrax for the same numbers over /api/portal/briefings, because it has no
 * log of its own.)
 *
 * ONE SESSION IS ONE GROUP. `foldBriefingLog` above emits one record per (room,
 * session) — correct there, because two rooms are two occupancies — but a
 * Mega-night group sent to both rooms was briefed ONCE, by one person, so this
 * counts DISTINCT sessions and not records.
 *
 * `unattributed` is distinct sessions with no host: rows from before hosts were
 * recorded (2026-09-03) and sends nobody typed a punch ID for. It is REPORTED
 * rather than dropped, because a night with eight anonymous groups is a
 * different fact from a quiet night, and only one of them needs looking into.
 *
 * The buckets are keyed by whatever `host` each record carries, which mirrors
 * the endpoint's `GROUP BY staff_user_id`. Every record of one session carries
 * the SAME host — the board joins it on by sessionId, the endpoint stores it on
 * every row of the session — so no session can land in two buckets, and
 * `sum(hosts) + unattributed === groups` holds for any input the board can
 * actually produce.
 */
export function briefedTodayByHost(
  briefings: ReadonlyArray<{ host: string | null; sessionId: string }>,
): {
  hosts: Array<{ host: string; briefed: number }>;
  unattributed: number;
  groups: number;
} {
  ...
  for (const b of briefings) {
    allSessions.add(b.sessionId);
    if (b.host == null) {
      anonymous.add(b.sessionId);
      continue;
    }
    const seen = byHost.get(b.host);
    if (seen) seen.add(b.sessionId);
    else byHost.set(b.host, new Set([b.sessionId]));
  }

  const hosts = [...byHost.entries()]
    .map(([host, sessions]) => ({ host, briefed: sessions.size }))
    // Most groups first — the strip is read for "who is carrying tonight".
    // Name breaks the tie so the order is stable between polls.
    .sort((a, b) => b.briefed - a.briefed || a.host.localeCompare(b.host));

  return { hosts, unattributed: anonymous.size, groups: allSessions.size };
}
