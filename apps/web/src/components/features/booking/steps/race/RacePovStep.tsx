 ...
 * the extras.
 *
 * qty=0 POV state: "Add for all N racers — $X" primary button + a "No thanks"
 ...
 * qty>0 state: -/+ stepper + count + total + "Set to all" helper. The stepper
 * caps at the uncovered racer count (owner 2026-08-10 — one camera per
 * racer): + disables at max with a "Max N" hint, and a stale persisted qty
 * above the cap normalizes on mount. Combos (which legitimately set
 * povQuantity above party size) never render this step — hiddenInCombo.
 *
              ...
              <>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/20 text-lg text-white/50 transition-colors hover:border-white/40 hover:text-white disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:border-white/20 disabled:hover:text-white/50"
                    >
                      +
                    </button>
                ...
                {atMax ? (
                  <p className="text-center text-xs text-white/30">
                    {t("pov.maxHint", { count: offerCount })}
                  </p>
                ) : (
                  qty !== offerCount && (
                    <button
